...
int collision_record_only(struct reb_simulation* const r, struct reb_collision c){
    ...
    if (c.p1 == 1 || c.p2 == 1) {
        ...
        if (c.p1 == 1) {
            ...
            FILE* ofae = fopen("/output_dir/Gtype_delta50/collision_ae_particle2.txt","a+");        // open file for collision output
            fprintf(ofae, "%e\t", t);
            ...
            FILE* of = fopen("/output_dir/Gtype_delta50/collision_velocities_particle2.txt","a+");        // open file for collision output
            fprintf(of, "%e\t", t);                    // time
        ...
        if (c.p1 == 2) {
            ...
            FILE* ofae = fopen("/output_dir/Gtype_delta50/collision_ae_particle1.txt","a+");        // open file for collision output
            fprintf(ofae, "%e\t", t);
            fprintf(ofae, "%d\t", particles[c.p2].hash);                    // time, hash
            fprintf(ofae, "%e %e %e\t", o.a, o.e, o.inc);    // relative velocity
            fprintf(ofae, "\n");
            ...
            FILE* of = fopen("/output_dir/Gtype_delta50/collision_velocities_particle1.txt","a+");        // open file for collision output
            fprintf(of, "%e\t", t);                    // time
            fprintf(of, "%e %e %e\t", rel_vel, tp_vel,plan_vel);    // relative velocity
            fprintf(of, "\n");
            fclose(of);                        // close file
...
void heartbeat(struct reb_simulation* r){
    ...
    if (reb_output_check(r, 1000.*r->dt)){
        ...
        for (int i=r->N_active;i<r->N;i++){
            ...
            // Removing test-particles with pericentre more than 10 *  outer-planet's semi-major axis
