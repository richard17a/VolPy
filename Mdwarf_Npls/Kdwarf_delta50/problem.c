...
int collision_record_only(struct reb_simulation* const r, struct reb_collision c){
    ...
    if (c.p1 == 1 || c.p2 == 1) {
        ...
        if (c.p1 == 1) {
            ...
            FILE* ofae = fopen("/data/rja92/rebound_outputs/stellar_type_N_pl_experiment/Kdwarf_delta50/collision_ae_particle2.txt","a+");        // open file for collision output
            fprintf(ofae, "%e\t", t);
            ...
            FILE* of = fopen("/data/rja92/rebound_outputs/stellar_type_N_pl_experiment/Kdwarf_delta50/collision_velocities_particle2.txt","a+");        // open file for collision output
            fprintf(of, "%e\t", t);                    // time
        ...
        if (c.p1 == 2) {
            ...
            FILE* ofae = fopen("/data/rja92/rebound_outputs/stellar_type_N_pl_experiment/Kdwarf_delta50/collision_ae_particle1.txt","a+");        // open file for collision output
            fprintf(ofae, "%e\t", t);
            fprintf(ofae, "%d\t", particles[c.p2].hash);                    // time, hash
            fprintf(ofae, "%e %e %e\t", o.a, o.e, o.inc);    // relative velocity
            fprintf(ofae, "\n");
            ...
            FILE* of = fopen("/data/rja92/rebound_outputs/stellar_type_N_pl_experiment/Kdwarf_delta50/collision_velocities_particle1.txt","a+");        // open file for collision output
            fprintf(of, "%e\t", t);                    // time
            fprintf(of, "%e %e %e\t", rel_vel, tp_vel,plan_vel);    // relative velocity
            fprintf(of, "\n");
            fclose(of);                        // close file
    ...
    if (reb_output_check(r, 1000.*r->dt)){
        ...
        for (int i=r->N_active;i<r->N;i++){
            ...
            if(o.a * (1 - o.e) > 10*a_p2){
                ...
                double amin = a_p2 * (1. + .2 * 2. * pow(m_p2/1., 2./7.)), amax = a_p2 * (1. + 1. * 2. * pow(m_p2/1., 2./7.));   //planet at inner edge of disk

                double a = reb_random_uniform(r, amin, amax);
                double T = 2.9;
                double e = pow(1 - (a_p2/a)*pow(T/2. - a_p2 / (2. * a), 2.), 1./2.);
